...
<html lang="es">
  <head>
    ...
    <title>Agregar rutina - Bitácora</title>
    <style>
        ...
        <form method="POST" action="{{ route('admin.rutinas.store') }}">
          ...
          <div class="grid">
            <div>
              ...
              <input id="title" name="title" value="{{ old('title') }}" placeholder="Nombre de la rutina" required />

              <label for="description">Descripción</label>
              <textarea id="description" name="description" rows="6" placeholder="Descripción breve">{{ old('description') }}</textarea>
            </div>

            <div>
              <label for="schedule">Horario / Programación</label>
              <input id="schedule" name="schedule" value="{{ old('schedule') }}" placeholder="Ej: Lunes 08:00, diario, semanal" />
              ...
              <input id="owner" name="owner" value="{{ old('owner') }}" placeholder="Nombre del responsable" />
            </div>
          </div>
